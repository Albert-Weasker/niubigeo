...
import { renderNiubigeoLockup } from "./brand.js";
...
(() => {
  ...
  const text = (value) => window.__niubigeoProductText ? window.__niubigeoProductText(String(value)) : String(value);
  const ui = (value) => esc(text(value));
  ...
  const requestButton = (label, action, primary) => '<button data-product-i18n type="button" class="p5-button' + (primary ? " primary" : "") + '" data-action="' + esc(action) + '">' + ui(label) + "</button>";
  const navButton = (label, target) => '<button data-product-i18n type="button" data-nav-target="' + esc(target) + '" class="' + (state.activeNav === target ? "active" : "") + '">' + ui(label) + "</button>";
  const modalShell = (content) => '<div class="p5-modalwrap" data-modal-backdrop>' + content + "</div>";
  const modalHeader = (title) => '<div class="p5-modal-head"><h2 data-product-i18n>' + esc(title) + '</h2>' + requestButton("关闭", "close") + "</div>";
  const searchLabel = (mode) => mode === "provider_native" ? "Provider 原生联网" : "不联网";
  const dateLabel = (value) => value ? new Date(value).toLocaleString("zh-CN", { hour12: false }) : text("尚未运行");
  ...
  const controlsFor = (definition) => definition[4] === "discovery"
    ? '<div class="p5-actions"><button data-product-i18n class="p5-button" data-view-metric="brand_name_mention" ' + (state.discoveryMetric === "brand_name_mention" ? "disabled" : "") + '>品牌名称</button><button class="p5-button" data-view-metric="domain_body_mention" ' + (state.discoveryMetric === "domain_body_mention" ? "disabled" : "") + ' data-product-i18n>回答正文域名</button></div>'
    : definition[4] === "association"
      ? '<div class="p5-actions"><button class="p5-button" data-view-metric="keyword_association_count" ' + (state.associationMetric === "keyword_association_count" ? "disabled" : "") + ' data-product-i18n>关联次数</button><button class="p5-button" data-view-metric="keyword_association_coverage" ' + (state.associationMetric === "keyword_association_coverage" ? "disabled" : "") + ' data-product-i18n>关联覆盖率</button><button class="p5-button" data-view-metric="keyword_relative_weight" ' + (state.associationMetric === "keyword_relative_weight" ? "disabled" : "") + ' data-product-i18n>相对权重</button></div>'
      : "";
  const chart = (definition, index) => {
    ...
    const context = definition[4] === "domain"
      ? object ? object.name + " · " + (object.domain || text("缺少域名")) : text("尚未选择对象")
      : definition[4] === "association"
        ? object && keyword ? object.name + " · " + keyword.keyword : text("尚未选择对象或关键词")
        : definition[4] === "gap"
          ? target && competitor && keyword ? target.name + text(" 与 ") + competitor.name + " · " + keyword.keyword : text("尚未选择可对照对象或关键词")
          : object && keyword ? object.name + " · " + keyword.keyword : text("尚未选择对象或关键词");
    if (!valid.length) return '<article class="p5-chart"><div class="p5-charthead"><div><h3>' + ui(definition[1]) + '</h3><p class="p5-formula">' + ui("对象或关键词：") + esc(context) + '</p><p class="p5-formula">' + ui("证明：") + ui(definition[2]) + '</p><p class="p5-formula">' + ui("计算：") + ui(definition[3]) + '</p></div>' + controlsFor(definition) + '</div><div class="p5-empty" data-product-i18n>尚未形成可计算数据。先保存监测范围并完成对应探针运行。</div></article>';
    ...
    const series = [...byModel.entries()].map(([seriesKey, rows]) => {
      ...
      return { color, pathData, latestPointId: latestComplete ? latestComplete.id : "", legend: '<span><i style="background:' + color + '"></i>' + esc(latest.modelDisplayName) + ' · ' + ui(searchLabel(latest.webSearchMode)) + ' · ' + latest.numerator + ' / ' + latest.denominator + '（' + ui(formattedValue(latest)) + '）' + (latest.complete ? "" : ui(" · 覆盖不完整")) + '</span>' };
    });
    ...
    const dots = [...pointGroups.values()].map((rows) => { const visible = rows.at(-1); const ids = rows.map((item) => item.id).join(","); const names = rows.map((item) => item.modelDisplayName).join("、"); const color = colorFor(visible.modelId); const tooltip = rows.map((item) => item.modelDisplayName + " · " + text(searchLabel(item.webSearchMode)) + " · " + new Date(item.observedAt).toLocaleString("zh-CN", { hour12: false }) + " · " + text(formattedValue(item)) + text(" · 命中 " + item.numerator + " / 可判定 " + item.denominator + " · 预定 " + item.planned + " · 失败或排除 " + item.failed) + " · " + text(item.complete ? "完整" : "覆盖不完整")).join("\n"); return '<g class="p5-pointbutton" data-point="' + esc(visible.id) + '" data-points="' + esc(ids) + '" role="button" tabindex="0" aria-label="' + ui("查看") + esc(names) + ' · ' + ui(definition[1]) + ui("证据") + '"><title>' + esc(tooltip) + '</title><circle class="p5-point' + (visible.complete ? "" : " partial") + (rows.some((item) => latestPointIds.has(item.id)) ? " p5-latest" : "") + '" cx="' + x(visible).toFixed(1) + '" cy="' + y(visible).toFixed(1) + '" r="' + (rows.length > 1 ? "7" : "5") + '" fill="' + (visible.complete ? color : "#050505") + '" stroke="' + color + '"></circle></g>'; }).join("");
    const rows = points.map((item) => '<tr><td>' + esc(new Date(item.observedAt).toLocaleString("zh-CN", { hour12: false })) + '</td><td>' + esc(item.modelDisplayName) + '</td><td data-product-i18n>' + ui(searchLabel(item.webSearchMode)) + '</td><td>' + ui(formattedValue(item)) + '</td><td>' + item.numerator + ' / ' + item.denominator + '</td><td>' + item.planned + '</td><td>' + item.failed + '</td><td>' + text(item.complete ? "完整" : "覆盖不完整") + '</td></tr>').join("");
    return '<article class="p5-chart"><div class="p5-charthead"><div><h3>' + ui(definition[1]) + '</h3><p class="p5-formula">' + ui("对象或关键词：") + esc(context) + '</p><p class="p5-formula">' + ui("证明：") + ui(definition[2]) + '</p><p class="p5-formula">' + ui("计算：") + ui(definition[3]) + '</p><p class="p5-formula" data-product-i18n>每条线代表一个固定模型、实际联网方式与探针指纹；每个点代表一次真实运行。悬停或聚焦数据点可查看完整统计。</p></div>' + controlsFor(definition) + '</div><svg viewBox="0 0 ' + width + ' ' + height + '" role="img" aria-label="' + ui(definition[1] + "数据点") + '"><line class="p5-axis" x1="' + left + '" x2="' + (width - right) + '" y1="' + (height - bottom) + '" y2="' + (height - bottom) + '"></line><line class="p5-axis" x1="' + left + '" x2="' + (width - right) + '" y1="' + ((height - bottom + top) / 2) + '" y2="' + ((height - bottom + top) / 2) + '"></line><line class="p5-axis" x1="' + left + '" x2="' + (width - right) + '" y1="' + top + '" y2="' + top + '"></line>' + lines + dots + labels + '</svg><div class="p5-legend">' + series.map((item) => item.legend).join("") + '<span data-product-i18n>点击数据点查看组成数据和原始回答</span></div><details class="p5-table-detail"><summary data-product-i18n>查看图表数据</summary><table class="p5-table"><thead><tr><th data-product-i18n>时间</th><th data-product-i18n>模型</th><th data-product-i18n>执行方式</th><th data-product-i18n>结果</th><th data-product-i18n>命中 / 可判定</th><th data-product-i18n>预定</th><th data-product-i18n>失败或排除</th><th data-product-i18n>覆盖</th></tr></thead><tbody>' + rows + '</tbody></table></details></article>';
  };
  const metricCards = () => metricDefinitions().slice(0, 4).map((definition) => {
    ...
    return '<article class="p5-card"><span>' + ui(definition[1]) + '</span><strong>' + (latest ? latest.numerator + ' / ' + latest.denominator : ui("暂无数据")) + '</strong><small class="p5-status">' + (latest ? ui(formattedValue(latest)) + " · " : "") + ui(definition[3]) + '</small></article>';
  }).join("");
  const runSummary = () => {
    const run = state.runs[0];
    if (!run) return '<div class="p5-empty" data-product-i18n>尚未创建测量运行。保存范围后可以开始多模型认知与中性关键词测试。</div>';
    const models = (run.modelRuns || []).map((item) => '<tr><td>' + esc(item.modelSnapshot.displayName) + '</td><td data-product-i18n>' + ui(searchLabel(item.modelSnapshot.webSearchMode)) + '</td><td><span class="p5-status ' + (item.status === "completed" ? "p5-ok" : item.status === "running" ? "p5-warn" : "p5-bad") + '">' + ui(item.status) + '</span></td><td>' + (item.probeRunIds ? item.probeRunIds.length : 0) + '</td></tr>').join("");
    return '<table class="p5-table"><thead><tr><th data-product-i18n>模型</th><th data-product-i18n>联网方式</th><th data-product-i18n>状态</th><th data-product-i18n>探针</th></tr></thead><tbody>' + models + '</tbody></table>';
  };
  const watchSetSummary = () => {
    if (!state.watchSet) return '<div class="p5-empty" data-product-i18n>尚未保存监测范围。范围只使用已存认知档案中的候选对象和关键词；身份不明确的对象不会被独立域名测试。</div>';
    return '<div><p class="p5-muted">' + ui("当前版本 v" + state.watchSet.version + " · " + state.watchSet.repetitions + " 次重复 · 固定后不会改写历史运行。") + '</p><div>' + state.watchSet.objects.map((item) => '<span class="p5-tag">' + esc(item.name) + (item.domain ? " · " + esc(item.domain) : ui(" · 待确认身份")) + '</span>').join("") + '</div><div>' + state.watchSet.keywords.map((item) => '<span class="p5-tag">' + esc(item.keyword) + (item.neutralEligible ? "" : ui(" · 不用于中性测试")) + '</span>').join("") + '</div></div>';
  };
  const scopeControls = () => state.watchSet ? '<section class="p5-section"><div class="p5-charthead"><div><h2 data-product-i18n>查看范围</h2><p class="p5-muted" data-product-i18n>筛选仅读取已保存证据，不会发起模型请求。管理模型会改变后续运行；这里仅控制图例。</p></div><label class="p5-check" data-product-i18n><input type="checkbox" data-role="historical-models" ' + (state.showHistoricalModels ? "checked" : "") + '>显示历史停用模型</label></div><div class="p5-row"><label data-product-i18n>对象<select class="p5-select" data-role="measurement-object">' + state.watchSet.objects.map((item) => '<option value="' + esc(item.id) + '" ' + (item.id === (selectedObject()?.id || "") ? "selected" : "") + '>' + esc(item.name) + (item.domain ? " · " + esc(item.domain) : ui(" · 缺少域名")) + '</option>').join("") + '</select></label><label data-product-i18n>中性关键词<select class="p5-select" data-role="measurement-keyword">' + state.watchSet.keywords.filter((item) => item.neutralEligible).map((item) => '<option value="' + esc(item.id) + '" ' + (item.id === (selectedKeyword()?.id || "") ? "selected" : "") + '>' + esc(item.keyword) + '</option>').join("") + '</select></label><label data-product-i18n>时间范围<select class="p5-select" data-role="measurement-range"><option value="all" ' + (state.timeRange === "all" ? "selected" : "") + ' data-product-i18n>全部时间</option><option value="7" ' + (state.timeRange === "7" ? "selected" : "") + ' data-product-i18n>过去 7 天</option><option value="30" ' + (state.timeRange === "30" ? "selected" : "") + ' data-product-i18n>过去 30 天</option><option value="90" ' + (state.timeRange === "90" ? "selected" : "") + ' data-product-i18n>过去 90 天</option></select></label><label data-product-i18n>执行方式<select class="p5-select" data-role="measurement-search"><option value="all" ' + (state.searchFilter === "all" ? "selected" : "") + ' data-product-i18n>全部联网方式</option><option data-product-i18n value="off" ' + (state.searchFilter === "off" ? "selected" : "") + '>不联网</option><option data-product-i18n value="provider_native" ' + (state.searchFilter === "provider_native" ? "selected" : "") + '>Provider 原生联网</option></select></label><label data-product-i18n>数据来源<select class="p5-select" data-role="measurement-source"><option value="all" ' + (state.sourceFilter === "all" ? "selected" : "") + ' data-product-i18n>手动与定时</option><option value="manual" ' + (state.sourceFilter === "manual" ? "selected" : "") + ' data-product-i18n>手动运行</option><option value="scheduled" ' + (state.sourceFilter === "scheduled" ? "selected" : "") + ' data-product-i18n>定时运行</option></select></label></div></section>' : "";
  const taskSummary = () => state.tasks.length ? state.tasks.filter((task) => task.status !== "deleted").map((task) => '<article class="p5-card"><span>' + esc(task.name) + ' · ' + ui(task.status === "active" ? "运行中" : task.status === "paused" ? "已暂停" : "配置不兼容") + '</span><strong>' + ui(task.rule.frequency === "daily" ? "每天" : task.rule.frequency === "weekly" ? "每周" : task.rule.frequency === "monthly" ? "每月" : "自定义") + '</strong><small class="p5-status">' + ui(task.modelScope.length + " 个模型 · " + task.plannedRequestCount + " 个请求 · 下次：") + esc(dateLabel(task.nextRunAt)) + '</small><div class="p5-actions">' + requestButton("预览", "task-preview:" + task.id) + (task.status === "active" ? requestButton("暂停", "task-pause:" + task.id) : task.status === "paused" ? requestButton("恢复", "task-resume:" + task.id) : "") + requestButton("编辑", "task-edit:" + task.id) + (state.deleteTaskId === task.id ? requestButton("确认删除", "task-confirm-delete:" + task.id) : requestButton("删除", "task-delete:" + task.id)) + '</div>' + ((state.taskPreview[task.id] || []).length ? '<div class="p5-status">' + ui("下三次：") + state.taskPreview[task.id].map((value) => esc(dateLabel(value))).join(" · ") + '</div>' : "") + '</article>').join("") : '<div class="p5-empty" data-product-i18n>尚未设置定时监测。任务只在固定监测配置和范围下运行，不会混合历史配置。</div>';
  const configurationSummary = () => {
    ...
    const action = status === "unchanged" ? '<button class="p5-button" disabled data-product-i18n>✓ 当前配置已保存</button>' : requestButton(status === "changed" ? "保存为新监测配置" : "保存监测配置", "save-configuration");
    return '<div class="p5-alert"><strong>' + ui(label) + '</strong><p class="p5-muted">' + ui(status === "unchanged" ? "当前模型、联网方式、域名和语言与已保存版本一致。" : "保存后会创建新版本；旧运行和历史证据不会被改写。") + '</p><div class="p5-actions">' + action + '</div></div>';
  };
  const overview = () => '<div class="p5-heading" id="p5-overview"><div><h1 data-product-i18n>持续测量</h1><p class="p5-muted" data-product-i18n>每条线只连接同一项目、监测配置、模型、联网方式和协议下的真实样本。刷新页面不会调用模型。</p></div><div class="p5-actions">' + requestButton("管理模型", "models") + requestButton("保存监测范围", "watchset") + requestButton("开始认知测试", "run", true) + '</div></div>' + configurationSummary() + (state.error ? '<div class="p5-alert"><strong data-product-i18n>操作未完成</strong><p class="p5-muted">' + ui(state.error) + '</p></div>' : "") + scopeControls() + '<section class="p5-section"><h2 data-product-i18n>当前数据</h2><div class="p5-grid">' + metricCards() + '</div></section><section class="p5-section" id="p5-competition"><h2 data-product-i18n>AI 对你品牌的回答变化</h2><p class="p5-muted" data-product-i18n>每个点代表一次完整运行。每条线只代表一个模型；不同协议、联网方式或配置指纹不会被连成同一条线。</p>' + metricDefinitions().map(chart).join("") + '</section><section class="p5-section" id="p5-keywords"><h2 data-product-i18n>监测范围</h2>' + watchSetSummary() + '</section><section class="p5-section" id="p5-recognition"><div class="p5-charthead"><div><h2 data-product-i18n>本次运行</h2><p class="p5-muted" data-product-i18n>一个模型失败不会覆盖其他模型的证据。可在单个探针中查看和重试。</p></div>' + requestButton("只测试新增模型", "new-models") + '</div>' + runSummary() + '</section><section class="p5-section" id="p5-monitoring"><div class="p5-charthead"><div><h2 data-product-i18n>定时监测</h2><p class="p5-muted" data-product-i18n>计划任务复用相同的测量执行路径，并记录每次发生的运行。</p></div>' + requestButton("设置定时监测", "schedule") + '</div><div class="p5-grid">' + taskSummary() + '</div></section>';
  const createModal = () => modalShell('<form class="p5-modal" data-form="project" role="dialog" aria-modal="true">' + modalHeader("新建项目") + '<p class="p5-muted" data-product-i18n>每个项目只绑定自己的域名、模型、范围、运行和任务。</p><label data-product-i18n>域名<input class="p5-input" name="domain" required placeholder="example.com"></label><label data-product-i18n>项目名称<input class="p5-input" name="name" data-product-i18n-placeholder placeholder="可选"></label><div class="p5-actions" style="margin-top:16px">' + requestButton("取消", "close") + '<button class="p5-button primary" type="submit" data-product-i18n>创建草稿项目</button></div></form>');
  const watchsetModal = () => state.watchSet ? modalShell('<div class="p5-modal" role="dialog" aria-modal="true">' + modalHeader("当前监测范围") + '<p class="p5-muted" data-product-i18n>当前范围已保存。模型、关键词和对象的历史证据保留在原版本中；配置改变后需要新的范围版本。</p><div class="p5-actions">' + requestButton("关闭", "close") + "</div></div>") : modalShell('<div class="p5-modal" role="dialog" aria-modal="true">' + modalHeader("保存监测范围") + '<p class="p5-muted" data-product-i18n>系统会从该项目已保存的认知档案中提出对象和关键词候选，不会猜测竞争对象域名。</p><div class="p5-actions">' + requestButton("取消", "close") + requestButton("保存并确认范围", "create-watchset", true) + "</div></div>");
  const modelModal = () => {
    const selectedIds = new Set(state.selections.map((item) => item.modelId));
    const rows = state.catalog.map((model) => {
      const selected = state.selections.find((item) => item.modelId === model.modelId);
      const searchable = ((model.displayName || model.modelId) + " " + model.modelId).toLocaleLowerCase();
      return '<label class="p5-check" data-model-row data-search="' + esc(searchable) + '"><input type="checkbox" data-model="' + esc(model.modelId) + '" ' + (selected ? "checked" : "") + '><span class="p5-model-copy"><strong>' + esc(model.displayName || model.modelId) + '</strong><small>' + esc(model.modelId) + '</small></span><select class="p5-select" data-product-i18n-aria-label aria-label="联网方式" data-mode="' + esc(model.modelId) + '"><option data-product-i18n value="off" ' + (selected && selected.webSearchMode === "off" ? "selected" : "") + '>不联网</option><option data-product-i18n value="provider_native" ' + (selected && selected.webSearchMode === "provider_native" ? "selected" : "") + (model.nativeWebSearchSupported ? "" : " disabled") + '>Provider 原生联网</option></select></label>';
    }).join("");
    return modalShell('<div class="p5-modal p5-model-manager" role="dialog" aria-modal="true" aria-labelledby="model-manager-title"><div class="p5-modal-head"><div><h2 id="model-manager-title" data-product-i18n>管理模型</h2><p class="p5-muted" data-product-i18n>选择后续运行的模型，并设置每个模型的联网方式。</p></div>' + requestButton("关闭", "close") + '</div><div class="p5-model-toolbar"><label><span data-product-i18n>搜索模型</span><input class="p5-input" data-role="model-query" value="' + esc(state.modelQuery) + '" data-product-i18n-placeholder placeholder="模型名称或标识符"></label><label class="p5-selected-filter"><input type="checkbox" data-role="selected-models-only" ' + (state.modelSelectedOnly ? "checked" : "") + '><span data-product-i18n>仅显示已选模型</span></label><span class="p5-status"><span data-product-i18n>已选模型</span>: <span data-role="selected-model-count">' + selectedIds.size + '</span></span></div><div class="p5-model-list">' + rows + '<div class="p5-empty" data-role="model-filter-empty" data-product-i18n hidden>没有符合筛选条件的模型。</div></div><div class="p5-modal-footer">' + requestButton("取消", "close") + requestButton("保存模型选择", "save-models", true) + '</div></div>');
  };
  const scheduleModal = () => {
    ...
    const preview = state.schedulePreview.length ? '<div class="p5-empty"><strong data-product-i18n>下三次执行</strong><div>' + state.schedulePreview.map((value) => esc(dateLabel(value))).join("<br>") + '</div></div>' : "";
    return '<div class="p5-modalwrap"><form class="p5-modal" data-form="schedule" data-task-id="' + esc(task?.id || "") + '"><h2>' + ui(task ? "编辑定时监测" : "设置定时监测") + '</h2><p class="p5-muted" data-product-i18n>任务使用冻结的监测配置和范围。改动未来安排，不改写已经保存的运行和证据。</p><label data-product-i18n>任务名称<input class="p5-input" name="name" value="' + esc(task?.name || text("定期监测")) + '"></label><div class="p5-row"><label data-product-i18n>频率<select class="p5-select" name="frequency"><option value="daily" ' + (rule.frequency === "daily" ? "selected" : "") + ' data-product-i18n>每天</option><option value="weekly" ' + (isWeekly ? "selected" : "") + ' data-product-i18n>每周</option><option value="monthly" ' + (isMonthly ? "selected" : "") + ' data-product-i18n>每月</option><option value="custom" ' + (isCustom ? "selected" : "") + ' data-product-i18n>自定义 Cron</option></select></label><label data-product-i18n>时区<input class="p5-input" name="timezone" value="' + esc(rule.timezone) + '"></label></div><div class="p5-row" data-simple-schedule ' + (isCustom ? "hidden" : "") + '><label data-product-i18n>小时<input class="p5-input" name="hour" type="number" min="0" max="23" value="' + esc(rule.hour === undefined ? 9 : rule.hour) + '"></label><label data-product-i18n>分钟<input class="p5-input" name="minute" type="number" min="0" max="59" value="' + esc(rule.minute === undefined ? 0 : rule.minute) + '"></label></div><div class="p5-row" data-weekly-schedule ' + (isWeekly ? "" : "hidden") + '><label data-product-i18n>每周日期<select class="p5-select" name="weekday">' + [1, 2, 3, 4, 5, 6, 0].map((weekday) => '<option value="' + weekday + '" ' + (rule.weekday === weekday ? "selected" : "") + '>' + ui(["周日", "周一", "周二", "周三", "周四", "周五", "周六"][weekday]) + '</option>').join("") + '</select></label><div></div></div><div class="p5-row" data-monthly-schedule ' + (isMonthly ? "" : "hidden") + '><label data-product-i18n>每月日期<input class="p5-input" name="dayOfMonth" type="number" min="1" max="31" value="' + esc(rule.dayOfMonth === undefined ? 1 : rule.dayOfMonth) + '"></label><div></div></div><label data-custom-schedule ' + (isCustom ? "" : "hidden") + '>Cron<input class="p5-input" name="cron" value="' + esc(rule.cron || "") + '"></label><h3 data-product-i18n>本任务执行的模型</h3><p class="p5-muted" data-product-i18n>模型范围固定在任务版本内。模型变更后，旧任务会在下一次执行前检查兼容性。</p><div>' + state.selections.map((selection) => '<label class="p5-check"><input type="checkbox" data-schedule-model="' + esc(selection.modelId) + '" ' + (modelScope.has(selection.modelId) ? "checked" : "") + '><span>' + esc(selection.displayName) + ' · ' + ui(searchLabel(selection.webSearchMode)) + '</span></label>').join("") + '</div>' + preview + '<div class="p5-actions" style="margin-top:16px">' + requestButton("预览下三次", "schedule-preview") + requestButton("取消", "close") + '<button class="p5-button primary" type="submit">' + ui(task ? "保存任务" : "创建监测任务") + '</button></div></form></div>';
  };
  const drawer = () => {
    if (!state.detail.length) return "";
    return '<aside class="p5-drawer" data-testid="measurement-evidence-drawer"><div class="p5-charthead"><strong data-product-i18n>数据点证据</strong>' + requestButton("关闭", "close-drawer") + '</div>' + state.detail.map((detail) => { const samples = detail.samples || []; return '<section class="p5-section"><p class="p5-muted">' + ui("模型：") + esc(detail.point.modelDisplayName) + ' · ' + ui(searchLabel(detail.point.webSearchMode)) + '</p><p class="p5-muted">' + ui("分子 " + detail.point.numerator + " / 分母 " + detail.point.denominator + " · 预定 " + detail.point.planned + " · 失败或排除 " + detail.point.failed) + '</p>' + samples.map((sample) => { const attempt = sample.detail && sample.detail.attempts ? sample.detail.attempts.find((item) => item.id === sample.sample.attemptId) || null : null; return '<article class="p5-card" style="margin-top:10px"><span>' + ui(sample.sample.included ? sample.sample.numerator ? "计入分子" : "计入分母" : "已排除") + '</span><p class="p5-muted">' + (sample.sample.exclusionReason ? esc(sample.sample.exclusionReason) : ui("可判定")) + '</p><p class="p5-status">Probe：' + esc(sample.sample.probeRunId) + ' · Attempt：' + (sample.sample.attemptId ? esc(sample.sample.attemptId) : ui("未创建")) + '</p>' + (attempt && attempt.rawAnswer ? '<pre>' + esc(attempt.rawAnswer) + '</pre>' : '<p class="p5-muted" data-product-i18n>此样本没有可展示的原始回答。</p>') + '</article>'; }).join("") + '</section>'; }).join("") + '</aside>';
  };
  const render = () => {
    const project = currentProject();
    const headerAction = state.loading ? '<button type="button" class="p5-button primary" disabled data-product-i18n>正在读取…</button>' : requestButton("新建项目", "create", true);
    const body = state.loading ? '<div class="p5-empty" data-testid="phase5-loading" data-product-i18n>正在读取项目数据，不会重新调用模型。</div>' : '<div data-testid="phase5-ready">' + (project ? overview() : '<div class="p5-empty"><h2 data-product-i18n>还没有项目</h2><p data-product-i18n>新建一个域名项目后，模型、监测范围、运行、图表和任务都会严格归属到这个项目。</p>' + requestButton("新建项目", "create", true) + '</div>') + '</div>';
    mount.innerHTML = css() + '<div class="p5-shell" data-testid="phase5-workbench"><aside class="p5-side"><div class="p5-brand">${phase5BrandLockup}</div><div data-product-i18n class="p5-label">项目</div><select class="p5-select" data-role="projects">' + (state.projects.length ? state.projects.map((item) => '<option value="' + esc(item.id) + '" ' + (item.id === state.projectId ? "selected" : "") + '>' + esc(item.name) + ' · ' + esc(item.normalizedDomain) + '</option>').join("") : '<option data-product-i18n>还没有项目</option>') + '</select><nav class="p5-nav">' + navButton("总览", "p5-overview") + navButton("域名认知", "p5-recognition") + navButton("竞争对照", "p5-competition") + navButton("关键词", "p5-keywords") + navButton("来源", "p5-competition") + navButton("监测", "p5-monitoring") + '</nav><div class="p5-label" data-product-i18n>数据来源</div><p class="p5-muted" style="padding:0 8px" data-product-i18n>Provider API 观察<br>筛选与图表不会调用模型。</p></aside><main class="p5-main"><header class="p5-header"><strong>NiubiGEO / ' + (project ? esc(project.name) : ui("项目")) + '</strong>' + headerAction + '</header><div class="p5-content">' + body + '</div></main></div>' + (state.modal === "create" ? createModal() : state.modal === "watchset" ? watchsetModal() : state.modal === "models" ? modelModal() : state.modal === "schedule" ? scheduleModal() : "") + drawer();
    if (state.modal === "models") filterModelRows();
  ...
  const perform = async (button, label, job) => {
    ...
    button.dataset.state = "loading"; button.textContent = text("◌ 正在" + label + "…"); button.disabled = true;
    try { await job(); button.dataset.state = "success"; button.textContent = text("✓ 已完成"); window.setTimeout(load, 800); }
    catch (_) { state.error = "操作未完成。请检查当前项目的配置、范围、模型、预算或任务状态。"; button.dataset.state = "error"; button.textContent = text("操作失败 · 重试"); button.disabled = false; render(); }
  };
  ...
  const filterModelRows = () => {
    const manager = document.querySelector(".p5-model-manager");
    if (!manager) return;
    const queryInput = manager.querySelector('[data-role="model-query"]');
    const selectedOnlyInput = manager.querySelector('[data-role="selected-models-only"]');
    state.modelQuery = queryInput instanceof HTMLInputElement ? queryInput.value : "";
    state.modelSelectedOnly = selectedOnlyInput instanceof HTMLInputElement && selectedOnlyInput.checked;
    const query = state.modelQuery.trim().toLocaleLowerCase();
    let selectedCount = 0;
    let visibleCount = 0;
    manager.querySelectorAll('[data-model-row]').forEach((row) => {
      const checkbox = row.querySelector('input[data-model]');
      const checked = checkbox instanceof HTMLInputElement && checkbox.checked;
      if (checked) selectedCount += 1;
      row.hidden = Boolean((query && !(row.getAttribute("data-search") || "").includes(query)) || (state.modelSelectedOnly && !checked));
      if (!row.hidden) visibleCount += 1;
    });
    manager.querySelector('[data-role="selected-model-count"]').textContent = String(selectedCount);
    manager.querySelector('[data-role="model-filter-empty"]').hidden = visibleCount !== 0;
  };
  document.addEventListener("input", (event) => { const target = event.target; if (target instanceof HTMLInputElement && target.dataset.role === "model-query") filterModelRows(); });
  document.addEventListener("change", (event) => { const target = event.target; if (target instanceof HTMLInputElement && (target.dataset.role === "selected-models-only" || target.hasAttribute("data-model"))) filterModelRows(); });
  ...
  document.addEventListener("submit", async (event) => {
    ...
    if (!(form instanceof HTMLFormElement)) return;
    if (form.dataset.form === "project") { event.preventDefault(); const button = form.querySelector("button[type='submit']"); return perform(button, "创建项目", async () => { const data = new FormData(form); const uiLocale = localStorage.getItem("niubigeo.product.locale"); const defaultLanguage = uiLocale === "pt-BR" ? "pt-BR" : uiLocale === "en" ? "en" : "zh-CN"; const created = await api("/api/projects", { method: "POST", body: JSON.stringify({ domain: String(data.get("domain") || ""), name: String(data.get("name") || ""), defaultLanguage }) }); selectProject(created.project.id); state.modal = ""; }); }
