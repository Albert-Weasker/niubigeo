...
(() => {
  ...
  const requestButton = (label, action, primary) => '<button data-product-i18n type="button" class="p5-button' + (primary ? " primary" : "") + '" data-action="' + esc(action) + '">' + esc(label) + "</button>";
  const navButton = (label, target) => '<button data-product-i18n type="button" data-nav-target="' + esc(target) + '" class="' + (state.activeNav === target ? "active" : "") + '">' + esc(label) + "</button>";
  ...
  const polishRules = '.p5-heading h1,.p5-modal h2{font-weight:600;letter-spacing:-.015em}.p5-section h2,.p5-chart h3,.p5-card strong{font-weight:600}.p5-button{min-height:44px;padding:10px 16px;font-weight:500}.p5-card{background:#0d0d0d}.p5-card:hover{transform:none}.p5-alert{border-left-width:1px;background:#10100d}.p5-drawer{box-shadow:0 8px 8px #0006}.p5-check input[type="checkbox"],.p5-selected-filter input[type="checkbox"]{box-sizing:border-box;width:18px;height:18px;min-height:18px;flex:0 0 18px;accent-color:#4c8dff}.p5-model-manager{width:min(920px,calc(100vw - 48px));height:min(760px,calc(100vh - 48px));max-height:none;overflow:hidden;padding:0;display:grid;grid-template-rows:auto auto minmax(0,1fr) auto}.p5-model-manager .p5-modal-head{position:relative;top:auto;margin:0;padding:20px 24px 16px}.p5-model-manager .p5-modal-head h2{margin:0 0 4px}.p5-model-toolbar{display:grid;grid-template-columns:minmax(260px,1fr) auto auto;align-items:end;gap:16px;padding:16px 24px;border-bottom:1px solid #262626}.p5-model-toolbar label:first-child{display:grid;gap:7px;color:#a1a1aa;font-size:12px;font-weight:500}.p5-selected-filter{display:flex;align-items:center;gap:8px;min-height:44px;font-weight:400;color:#d4d4d8;white-space:nowrap}.p5-model-list{overflow:auto;padding:4px 24px}.p5-check[data-model-row]{display:grid;grid-template-columns:20px minmax(0,1fr) minmax(190px,260px);align-items:center;gap:14px;padding:12px 4px;border-bottom:1px solid #202020;font-weight:400}.p5-check[data-model-row][hidden]{display:none}.p5-model-copy{display:grid;gap:4px;min-width:0}.p5-model-copy strong{font-weight:500;overflow-wrap:anywhere}.p5-model-copy small{color:#71717a;overflow-wrap:anywhere}.p5-modal-footer{display:flex;justify-content:flex-end;gap:8px;padding:16px 24px;border-top:1px solid #262626;background:#111}@media(max-width:640px){.p5-heading,.p5-charthead{display:grid}.p5-grid,.p5-row{grid-template-columns:1fr}.p5-model-manager{width:calc(100vw - 24px);height:calc(100vh - 24px)}.p5-model-toolbar{grid-template-columns:1fr;gap:8px}.p5-check[data-model-row]{grid-template-columns:20px minmax(0,1fr)}.p5-check[data-model-row] .p5-select{grid-column:1/-1}.p5-modal-footer{padding:12px}}';
  const css = () => '<style>' + polishRules +
  ...
  const controlsFor = (definition) => definition[4] === "discovery"
    ? '<div class="p5-actions"><button data-product-i18n class="p5-button" data-view-metric="brand_name_mention" ' + (state.discoveryMetric === "brand_name_mention" ? "disabled" : "") + '>品牌名称</button><button class="p5-button" data-view-metric="domain_body_mention" ' + (state.discoveryMetric === "domain_body_mention" ? "disabled" : "") + '>回答正文域名</button></div>'
  ...
  const chart = (definition, index) => {
    ...
    const rows = points.map((item) => '<tr><td>' + esc(new Date(item.observedAt).toLocaleString("zh-CN", { hour12: false })) + '</td><td>' + esc(item.modelDisplayName) + '</td><td data-product-i18n>' + esc(searchLabel(item.webSearchMode)) + '</td><td>' + esc(formattedValue(item)) + '</td><td>' + item.numerator + ' / ' + item.denominator + '</td><td>' + item.planned + '</td><td>' + item.failed + '</td><td>' + (item.complete ? "完整" : "覆盖不完整") + '</td></tr>').join("");
  ...
  const runSummary = () => {
    ...
    const models = (run.modelRuns || []).map((item) => '<tr><td>' + esc(item.modelSnapshot.displayName) + '</td><td data-product-i18n>' + esc(searchLabel(item.modelSnapshot.webSearchMode)) + '</td><td><span class="p5-status ' + (item.status === "completed" ? "p5-ok" : item.status === "running" ? "p5-warn" : "p5-bad") + '">' + esc(item.status) + '</span></td><td>' + (item.probeRunIds ? item.probeRunIds.length : 0) + '</td></tr>').join("");
  ...
  const modelModal = () => {
    ...
    const rows = state.catalog.map((model) => { const selected = state.selections.find((item) => item.modelId === model.modelId); const searchable = ((model.displayName || model.modelId) + " " + model.modelId).toLocaleLowerCase(); return '<label class="p5-check" data-model-row data-search="' + esc(searchable) + '"><input type="checkbox" data-model="' + esc(model.modelId) + '" ' + (selected ? "checked" : "") + '><span class="p5-model-copy"><strong>' + esc(model.displayName || model.modelId) + '</strong><small>' + esc(model.modelId) + '</small></span><select class="p5-select" aria-label="Modo de acesso à web" data-mode="' + esc(model.modelId) + '"><option data-product-i18n value="off" ' + (selected && selected.webSearchMode === "off" ? "selected" : "") + '>不联网</option><option data-product-i18n value="provider_native" ' + (selected && selected.webSearchMode === "provider_native" ? "selected" : "") + (model.nativeWebSearchSupported ? "" : " disabled") + '>Provider 原生联网</option></select></label>'; }).join("");
    return modalShell('<div class="p5-modal p5-model-manager" style="width:min(920px,calc(100vw - 48px));height:min(760px,calc(100vh - 48px));max-height:none;overflow:hidden;padding:0;display:grid" role="dialog" aria-modal="true" aria-labelledby="model-manager-title"><div class="p5-modal-head"><div><h2 id="model-manager-title">Gerenciar modelos</h2><p class="p5-muted">Escolha os modelos das próximas execuções e defina como cada um acessa a web.</p></div>' + requestButton("关闭", "close") + '</div><div class="p5-model-toolbar"><label><span>Pesquisar modelos</span><input class="p5-input" data-role="model-query" value="' + esc(state.modelQuery) + '" placeholder="Nome ou identificador do modelo"></label><label class="p5-selected-filter"><input type="checkbox" data-role="selected-models-only" ' + (state.modelSelectedOnly ? "checked" : "") + '><span>Exibir somente selecionados</span></label><span class="p5-status">' + selectedIds.size + ' modelos selecionados</span></div><div class="p5-model-list">' + (rows || '<div class="p5-empty">Nenhum modelo corresponde aos filtros.</div>') + '</div><div class="p5-modal-footer">' + requestButton("取消", "close") + requestButton("保存模型选择", "save-models", true) + '</div></div>');
  };
  const scheduleModal = () => {
  ...
  const render = () => {
    ...
    const body = state.loading ? '<div class="p5-empty" data-testid="phase5-loading">正在读取项目数据，不会重新调用模型。</div>' : '<div data-testid="phase5-ready">' + (project ? overview() : '<div class="p5-empty"><h2 data-product-i18n>还没有项目</h2><p>新建一个域名项目后，模型、监测范围、运行、图表和任务都会严格归属到这个项目。</p>' + requestButton("新建项目", "create", true) + '</div>') + '</div>';
    mount.innerHTML = css() + '<div class="p5-shell" data-testid="phase5-workbench"><aside class="p5-side"><div class="p5-brand">${phase5BrandLockup}</div><div data-product-i18n class="p5-label">项目</div><select class="p5-select" data-role="projects">' + (state.projects.length ? state.projects.map((item) => '<option value="' + esc(item.id) + '" ' + (item.id === state.projectId ? "selected" : "") + '>' + esc(item.name) + ' · ' + esc(item.normalizedDomain) + '</option>').join("") : '<option data-product-i18n>还没有项目</option>') + '</select><nav class="p5-nav">' + navButton("总览", "p5-overview") + navButton("域名认知", "p5-recognition") + navButton("竞争对照", "p5-competition") + navButton("关键词", "p5-keywords") + navButton("来源", "p5-competition") + navButton("监测", "p5-monitoring") + '</nav><div class="p5-label">数据来源</div><p class="p5-muted" style="padding:0 8px">Provider API 观察<br>筛选与图表不会调用模型。</p></aside><main class="p5-main"><header class="p5-header"><strong>NiubiGEO / ' + esc(project ? project.name : "项目") + '</strong>' + headerAction + '</header><div class="p5-content">' + body + '</div></main></div>' + (state.modal === "create" ? createModal() : state.modal === "watchset" ? watchsetModal() : state.modal === "models" ? modelModal() : state.modal === "schedule" ? scheduleModal() : "") + drawer();
  ...
  const filterModelRows = () => {
    const queryInput = document.querySelector('[data-role="model-query"]');
    const selectedOnlyInput = document.querySelector('[data-role="selected-models-only"]');
    const query = queryInput instanceof HTMLInputElement ? queryInput.value.trim().toLocaleLowerCase() : "";
    const selectedOnly = selectedOnlyInput instanceof HTMLInputElement && selectedOnlyInput.checked;
    document.querySelectorAll('[data-model-row]').forEach((row) => { const checkbox = row.querySelector('input[data-model]'); row.hidden = Boolean((query && !(row.getAttribute("data-search") || "").includes(query)) || (selectedOnly && checkbox instanceof HTMLInputElement && !checkbox.checked)); });
  };
