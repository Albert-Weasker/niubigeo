...
function outputFor(modelId: string): string {
  if (modelId === "fixture/model-b") {
    return JSON.stringify({ analysisStatus: "unknown", domainRecognition: "unknown", recognizedBrand: { value: null, citationUrls: [] }, businessDescription: { value: null, citationUrls: [] }, productCategory: { value: null, citationUrls: [] }, competitors: [], brandKeywords: [], unknowns: ["本次无法确认 target.example。"] });
  }
  if (modelId === "fixture/model-c") {
    return JSON.stringify({
      analysisStatus: "recognized", domainRecognition: "recognized", recognizedBrand: { value: "TargetDocs", citationUrls: ["https://source.example/forge"] }, businessDescription: { value: "团队知识库工具", citationUrls: ["https://source.example/forge"] }, productCategory: { value: "协作工具", citationUrls: [] },
      competitors: [
      ...
      brandKeywords: [{ keyword: "API 文档", citationUrls: ["https://source.example/forge"] }, { keyword: "站内搜索", citationUrls: [] }], unknowns: [],
    });
  }
  return JSON.stringify({
    analysisStatus: "recognized", domainRecognition: "recognized", recognizedBrand: { value: "TargetDocs", citationUrls: [] }, businessDescription: { value: "开发者文档工具", citationUrls: [] }, productCategory: { value: "开发者工具", citationUrls: [] },
    competitors: [
    ...
    brandKeywords: [{ keyword: "API 文档", citationUrls: [] }, { keyword: "开源文档", citationUrls: [] }], unknowns: [],
  });
}
...
export class Phase4FixtureExecutor implements RecognitionAnswerExecutor {
  ...
  async execute(input: Parameters<RecognitionAnswerExecutor["execute"]>[0]): Promise<AnswerResult> {
    ...
    const text = modelId === "fixture/model-analysis-failed" ? "模型返回的非结构化原文 😀" : outputFor(modelId);
    return {
